...
$Maths = Read-Host "`nPlease enter the software name."

try 
{
    # Replace occurrences of "Maths" in all files
    Write-Host "`nReplacing occurrences of 'Maths' with "$Maths" in all files.`n"
    ...
    Get-ChildItem -File -Recurse | ForEach-Object {
        ...
        try
        {
            (Get-Content $_.FullName) -replace 'Maths', $Maths | Set-Content $_.FullName
        } 
        catch 
        {
    ...
    Write-Host "`nRenaming files and directories using "$Maths":`n"
    ...
    # Get all files and directories containing "Maths" recursively
    Get-ChildItem -Recurse -Directory | ForEach-Object {
        $dirpath = $_.FullName
        $dirname = Split-Path  $dirpath -Leaf
        if ($dirname.Contains("Maths"))
        {
            $stack.Push($dirpath)
        ...
        foreach ($file in [Directory]::EnumerateFiles($dirpath)) 
        {
            ...
            if ($filename.Contains('Maths') -and -not $allPaths.Contains($file))
            {
                $stack.Push($file)
                $allPaths.Add($file)
            }
    ...
    while ($stack.Count) {
        ...
        if($filename.Contains('Maths') -and $pathExists)
        {
            $newName = $filename.Replace('Maths', $Maths)
